...
interface TextTypeProps { 
  ...
  reverseMode?: boolean; 
  sentenceClassNames?: string[]; // Prop agregada
} 
...
const TextType = ({ 
  ...
  reverseMode = false, 
  sentenceClassNames = [], // Prop agregada
  ...props 
  ...
  const getCurrentSentenceClassName = () => {
    if (sentenceClassNames.length === 0) return '';
    return sentenceClassNames[currentTextIndex % sentenceClassNames.length] || '';
  }
  ...
  return createElement( 
    ...
    <span className={`inline ${getCurrentSentenceClassName()}`} style={{ color: getCurrentTextColor() }}> 
      {displayedText} 
    </span>, 
    showCursor && ( 
      <span 
